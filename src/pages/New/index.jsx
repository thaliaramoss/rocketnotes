...
export function New() {
  ...
  const [links, setLinks] = useState([]) // guarda links
  const [newLink, setNewLink] = useState("") // armazenar link que será adicionado
          ...
          <Section title="Links úteis">
            {
              links.map((link, index) => (
                <NoteItem 
                key={String(index)} // padrão de chave => formato de texto
                value={link}
                onClick={() =>handleRemoveLink(link)} // quando tem parâmetro se usa a arrow function
                />
              ))
            }
            <NoteItem 
